...
Detection runs for **every** site. Trusted server-side ingestion skips the header, client-signal, network and rate layers because those signals describe the reporting backend. The user-agent layer runs only when the event explicitly supplies `user_agent`.
...
Requests with a verified Bearer API key and `ingest:write` permission for the site are treated as trusted server-side ingestion. Backend-generated events that omit `user_agent` (or send an empty value) are not classified using the reporting HTTP client's `User-Agent`. If the event explicitly forwards a non-empty visitor or crawler `user_agent`, that value is still classified and `blockBots` still controls enforcement. An invalid or under-scoped credential receives none of these exemptions.
