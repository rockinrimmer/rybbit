...
Requests sent to `/api/track` with a verified API key that has `ingest:write` permission for the site are treated as trusted server-side ingestion. Four of the five layers are skipped: the headers, the client signals, the network address, and the request rate all belong to your server, not to the visitor it is reporting, so scoring them would convict your own backend.

For backend-generated events, omit `user_agent` (an empty value behaves the same way). Rybbit retains the request's HTTP `User-Agent` for metadata and identity, but does not classify that delivery client as the subject of the event. When you explicitly supply a non-empty `user_agent`, the user-agent layer runs on that value: real browsers are tracked normally, while recognised crawlers are detected. Invalid or under-scoped credentials do not receive the trusted-ingestion exemption.
